...
**执行任务**：根据题目原文、建模分析和代码实现的产出，撰写完整论文（不少于15000字）。

执行步骤：

- 询问用户使用docx格式还是latex格式进行撰写
- 检查用户是否提供论文模板
- 使用默认模板（`references/论文模板.docx` 或 `references/论文模板`）或用户模板撰写论文
- 使用对应的 skill 生成标准的论文
- 避免死板罗列，使用段落式自然陈述
- 完成所有标准章节
...
- **`references/论文模板/template.tex`** - 标准数学建模论文模板（.tex格式）
...
本技能集成了五个专业的文档处理子Skill，位于 `tools/` 目录下：
...
│   └── ref.bib
│
├── equation/                     # equation文件夹（用于存放公式）
│   ├── 1.afx                     # 公式1
│   ├── 2.afx                     # 公式2
│   └── ...                       # 其他公式
│
├── cumcmthesis.cls               # cls模板（只读）
├── 论文.tex                      # tex文件（论文撰写）
├── 论文.pdf                      # 目标论文（必须创建）
├── 建模手分析文档.md             # 建模手产出的分析文档（只读）
├── 术语表格.md                   # 建模手产出的术语表（只读）
